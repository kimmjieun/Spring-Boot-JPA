...
public class MemberApiController {
    ...
    /**
     * 등록 V2 : 요청값으로 Member 엔티티 대신에 별도의 DTO를 받는다
     */
     ...
     * 수정 API
     */
    @PutMapping("/api/v2/members/{id}") // post 나 patch가 더 알맞음
    ...
    @Data
    @AllArgsConstructor // dto 롬복 그냥 씀
     ...
     *  - 실무에서는 같은 엔티티에 대해 API가 용도에 따라 다양하게 만들어지는데 한 엔티티에 각각의 API를 위한 프레젠테이션 응답로직을 담기 어렵ㄷ바
     * - 엔티티가 변경되면 API 스펙이 변한다
     * - 추가로 컬렉션을 직접 변환하면 향후 API 스펙을 변경하기 어렵다 (별도의 Result 클래스 생성으로 해결)
     * 결론
